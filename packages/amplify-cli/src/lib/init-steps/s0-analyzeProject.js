...
async function getEnvName(context) {
  ...
  if (context.exeInfo.inputParams.amplify && context.exeInfo.inputParams.amplify.envName) {
    ...
    context.print.error('Environment name should be between 2 and 11 characters (only lowercase alphabets).');
    process.exit(1);
  ...
  const newEnvQuestion = async () => {
    const envNameQuestion = {
      ...
      validate: input => new Promise((resolvePromise, reject) => (!isEnvNameValid(input) ? reject(new Error('Environment name should be between 2 and 11 characters (only lowercase alphabets).')) : resolvePromise(true))),
    };
